...
export const transformVehicle = (item) => {
  ...
  const resolvedTypeId =
    item.vehicleTypeId ?? item.typeId ?? item.vehicleType?.id ?? null;

  return {
    id: item.vehicleId ?? item.id,
    code: item.licensePlate || `VH-${item.vehicleId ?? item.id}`,
    name: `${resolvedType || "Phương tiện"} ${item.licensePlate || ""}`.trim(),
    type: mapVehicleType(resolvedType),
    typeRaw: resolvedType, // giữ giá trị gốc để gọi API
    vehicleTypeId: resolvedTypeId,
    vehicleTypeName: item.vehicleTypeName ?? item.vehicleType?.name ?? null,
    model: item.model || "",
...
const vehicleService = {
  ...
  // Body: { depotId?, vehicleTypeId, model?, licensePlate, capacityPerson, status }
  createVehicle: async ({
    depotId,
    vehicleTypeId,
    type,
    ...
    try {
      ...
      if (vehicleTypeId !== undefined && vehicleTypeId !== null) {
        body.vehicleTypeId = Number(vehicleTypeId);
      } else if (type) {
