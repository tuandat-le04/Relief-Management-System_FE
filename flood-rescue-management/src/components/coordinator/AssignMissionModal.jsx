...
const AssignMissionModal = ({ isOpen, onClose, request, onSuccess }) => {
  ...
  const filteredTeams = useMemo(() => {
    ...
    return teamsWithDistance.filter((t) =>
  ...
  const handleAssignTeam = async () => {
    ...
    if (result.success) {
      ...
      if (msg.toLowerCase().includes("busy") || msg.toLowerCase().includes("bận")) {
        setTeamError("Đội cứu hộ đang bận, vui lòng chọn đội khác");
      } else if (msg.toLowerCase().includes("inactive") || msg.toLowerCase().includes("ngưng")) {
        setTeamError("Đội cứu hộ đang ngưng hoạt động");
      } else if (msg.toLowerCase().includes("not found") || msg.toLowerCase().includes("không tìm thấy")) {
  ...
  const handleAssignVehicle = async () => {
    ...
    if (result.success) {
      ...
      const msg = result.error || "";
      if (msg.toLowerCase().includes("in_use") || msg.toLowerCase().includes("maintenance")) {
  ...
  // ─── Submit tổng — gọi tuần tự team → vehicle ────────────────────────────
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[92vh] flex flex-col overflow-hidden">
        ...
        <div className="px-5 py-2.5 bg-blue-50 border-b border-blue-100 flex items-center gap-2 flex-shrink-0">
          ...
          <span className={`flex-shrink-0 text-xs font-bold px-2 py-0.5 rounded-full ${pConfig.bg} ${pConfig.text}`}>
            {pConfig.label}
          </span>
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-5">
            ...
            {loadingMission ? (
              ...
              <div className="bg-red-50 border border-red-300 rounded-xl px-4 py-2.5 flex items-center gap-2">
                ...
                <span className="text-sm text-red-700">
                  Không tìm thấy nhiệm vụ cho yêu cầu này. Vui lòng thử lại hoặc liên hệ Admin.
                </span>
              </div>
            )}
          </div>
          ...
          >
            {teamAssigned ? (
              ...
              <>
                ...
                {/* Danh sách đội */}
                {loadingTeams ? (
                  <LoadingRow label="Đang tải danh sách đội cứu hộ..." color="blue" />
                  ...
                  <div className="space-y-2 max-h-56 overflow-y-auto pr-1">
                    {filteredTeams.map((team) => {
                      ...
                      return (
                        ...
                        >
                          {/* Avatar */}
                          <div className={`w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0 ${isSelected ? "bg-blue-600" : "bg-slate-100"}`}>
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2">
                              <p className={`font-bold text-sm truncate ${isSelected ? "text-blue-800" : "text-slate-800"}`}>
                                {team.name}
                              </p>
                ...
                {selectedTeam && (
                  <div className="bg-blue-50 border border-blue-200 rounded-lg px-3 py-2 flex items-center gap-2">
                    ...
                    <span className="text-xs text-blue-800 font-semibold">
                      Đã chọn: <strong>{selectedTeam.name}</strong> (ID #{selectedTeam.id})
                    </span>
                  </div>
                )}
          ...
          >
            {vehicleAssigned ? (
              ...
              <>
                {loadingVehicles ? (
                  <LoadingRow label="Đang tải danh sách phương tiện..." color="purple" />
                ) : vehicles.length === 0 ? (
                  <div className="bg-amber-50 border border-amber-200 rounded-lg px-3 py-3 flex items-center gap-2">
                    <span className="material-symbols-outlined text-amber-500 text-base">no_transfer</span>
                    <div>
                      <p className="text-xs text-amber-700 font-bold">Hiện không có phương tiện sẵn sàng</p>
                      <p className="text-xs text-amber-600 mt-0.5">Tất cả xe đang bận hoặc bảo trì. Bạn có thể bỏ qua bước này.</p>
                    </div>
                  </div>
                ) : (
                  <div className="space-y-2 max-h-52 overflow-y-auto pr-1">
                    ...
                    {vehicles.map((v) => {
                      ...
                      return (
                        ...
                        >
                          <div className={`w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0 ${isSelected ? "bg-purple-600" : "bg-slate-100"}`}>
                            <span className={`material-symbols-outlined text-lg ${isSelected ? "text-white" : "text-slate-500"}`}>
                              {getVehicleIcon(v.typeRaw)}
                            </span>
                          </div>
                          <div className="flex-1 min-w-0">
                            <p className={`font-bold text-sm truncate ${isSelected ? "text-purple-800" : "text-slate-800"}`}>
                              {v.licensePlate || v.code}
                              {v.model && (
                                <span className="ml-1 font-normal text-slate-500 text-xs">— {v.model}</span>
                              )}
                            </p>
                          ...
                          <span className={`flex-shrink-0 text-[10px] font-bold px-2 py-0.5 rounded-full ${isSelected ? "bg-purple-600 text-white" : "bg-emerald-100 text-emerald-700"}`}>
                  ...
                  <div className="bg-purple-50 border border-purple-200 rounded-lg px-3 py-2 flex items-center justify-between gap-2">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs text-purple-800 font-semibold">
                        Đã chọn: <strong>{selectedVehicle.licensePlate || selectedVehicle.code}</strong> (ID #{selectedVehicle.id})
                      </span>
                    </div>
                    <button
        ...
        <div className="flex-shrink-0 px-5 py-3.5 border-t border-slate-200 bg-slate-50 flex items-center justify-between gap-3">
          ...
          <p className="text-xs text-slate-500 flex items-center gap-1.5">
            {isFullyDone ? (
              <>
                <span className="text-emerald-500 material-symbols-outlined text-sm">task_alt</span>
                Phân công hoàn tất
              </>
            ) : !selectedTeam ? (
              <>
                <span className="text-amber-500 material-symbols-outlined text-sm">info</span>
                Chưa chọn đội cứu hộ (bắt buộc)
              </>
            ) : (
              <>
                <span className="text-blue-500 material-symbols-outlined text-sm">pending</span>
                Đã chọn: {selectedTeam.name}
                {selectedVehicle ? ` + ${selectedVehicle.licensePlate || selectedVehicle.code}` : ""}
              </>
            )}
          </p>
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            ...
            {isFullyDone ? (
              ...
              <button
                ...
                disabled={!selectedTeam || !mission || loadingMission || loadingAssignTeam || loadingAssignVehicle}
              ...
              >
                {(loadingAssignTeam || loadingAssignVehicle) ? (
                  <>
      ...
      <div className="bg-slate-50 px-4 py-3 border-b border-slate-200 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <div className={`w-7 h-7 ${c.bg} rounded-lg flex items-center justify-center`}>
            <span className={`material-symbols-outlined ${c.text} text-base`}>{icon}</span>
          </div>
...
const StatusBadge = ({ status }) => {
  ...
  const label =
    status === "ACTIVE" ? "Sẵn sàng" : status === "BUSY" ? "Đang bận" : "Ngưng HĐ";
  return (
    <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-full flex-shrink-0 ${cls}`}>
      {label}
    </span>
  );
};
...
const LoadingRow = ({ label, color = "blue" }) => (
  <div className="flex items-center justify-center py-6 gap-2 text-slate-400 text-sm">
    <span className={`animate-spin rounded-full h-4 w-4 border-b-2 border-${color}-500`} />
    {label}
  </div>
);
...
const SuccessBanner = ({ message, color = "emerald" }) => {
  ...
  const [bg, border, iconColor, textColor] = colorMap[color]?.split(" ") || colorMap.emerald.split(" ");
  return (
    <div className={`${bg} border ${border} rounded-lg px-3 py-2.5 flex items-center gap-2`}>
      <span className={`material-symbols-outlined ${iconColor} text-base`}>check_circle</span>
