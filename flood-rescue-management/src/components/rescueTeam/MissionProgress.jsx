...
const normalizeSelectableStatus = (status) => {
  ...
  if (status === "IN_PROGRESS" || status === "ARRIVED" || status === "COMPLETED") {
    return status;
  }
  return "IN_PROGRESS";
};
...
export default function MissionProgress({
  ...
  return (
    <div className="bg-white dark:bg-[#1c1e22] border border-gray-200 dark:border-gray-700 rounded-2xl p-5">
      <div className="flex items-center justify-between gap-4">
        <div className="flex items-center gap-2">
          {steps.map((s) => {
            ...
            const isDisabled = submitting || isCompletedAlready || confirmedStep >= s.id;
      ...
      {error && (
        <p className="mt-3 text-xs text-red-600 font-bold">{error}</p>
      )}
    </div>
  );
}
